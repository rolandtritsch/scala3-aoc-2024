...
/** Day09 - Disk Fragmenter
  ...
  * - done
  * 
  * Note: Turns out that part2 required some serious refactoring.
  * But I still like this approach and the implementation. Means
...
object Day09p1 {
  ...
  def readFile(filename: String): Disk = {
    ...
    try {
      ...
      val (blocks, _) = line.tail.grouped(2).foldLeft(firstBlock, 1) { case ((bs, id), chars) => {
