...
/** Day08 - Resonant Collinearity
  ...
  *   - For every possible pair of antennas with the same frequency (and
  *     different positions) we need to calculate the pair of antinodes
  *   - We then need to count the antinodes (but just the ones that are within
  ...
  *   - we then just need to merge the antiNodePositions with the
  *     antennaPositions
  *   - note: Luckily we decided to use Sets (that way the merge will eliminate
  *     dublicates)
  *   - and count them
  *
  * Done.
...
object Day08:
    ...
    case class Antenna(val frequency: Char, val position: Position):
        ...
        private def positioning(a: Antenna): Positioning =
            if position.x > a.position.x && position.y > a.position.y then
                LOWER_RIGHT
            else if position.x > a.position.x && position.y < a.position.y then
                LOWER_LEFT
            else if position.x < a.position.x && position.y > a.position.y then
                UPPER_RIGHT
            else if position.x < a.position.x && position.y < a.position.y then
        ...
        def antiNode(a: Antenna): Position = positioning(a) match
            case LOWER_RIGHT =>
                Position(position.x + diffX(a), position.y + diffY(a))
            case LOWER_LEFT =>
                Position(position.x + diffX(a), position.y - diffY(a))
            case UPPER_RIGHT =>
                Position(position.x - diffX(a), position.y + diffY(a))
    ...
    object Grid:
        def isOnGrid(maxX: Int, maxY: Int, position: Position): Boolean =
            (0 until maxX).contains(position.x) &&
                (0 until maxY).contains(position.y)
    end Grid
