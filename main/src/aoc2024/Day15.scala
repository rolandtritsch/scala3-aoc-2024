...
/** Day15 - Warehouse Woes
  ...
  *
  * OK. We have a warehouse. With boxes in it and walls and a robot. The robot
  * can move around and push boxes. The robot can't move through walls or boxes.
  * The robot needs to move around and push boxes until it reaches the end of
  * the List of Moves it was given
  *
  * We can model this in a couple of ways ...
  *
  *   - Clearly we need a Position class to represent the positions of the robot
  *     and the boxes and the walls.
  *   - We need a Move class to represent the moves the robot needs to make.
  *   - We need a Warehouse class to represent the warehouse and the robot and
  *     the boxes and the walls.
  *   - Note: The Robot has no direction it (just) moves.
  *   - Note: We do not need to keep the dimensions of the warehouse, because
  *     the warehouse has an outer wall and the robot can't move through walls.
  *   - Note: I was wondering, if I want to make the moves part of the Warehouse
  *     class or the Robot class, but I feel it is cleaner to keep the moves
  *     separate and create a new abstraction called a State, which is a tuple
  *     of a Warehouse and Moves.
  *
  * One problem to solve while solving this problem is to push boxes, means if
  * the next position is blocked by a box and there is space between the box and
  * the wall, the box gets moved to that free space (which makes the next
  * position available for the robot to move to).
  *
  * When implementing this with a 2-dimensional array, we get the concept of
  * position-is-free/unoccupied for free (pun intended). When using list(s) of
  * positions we need to (explicitly) model the free-space as a list.
  *
  * Then we can ...
  *
  *   - look for free-space (between the current position and the wall in the
  ...
  *   - We need to execute on all moves. Every move will create a new State
  *     (Warehouse, Moves).
  *   - When we are out of Moves, we need to sum up the GPS coordinates of all
  *     the boxes.
  */
