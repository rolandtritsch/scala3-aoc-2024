...
/** Day18 - RAM Run
  *
  * Sounds like another dfs grid traversal problem to find the shortest path from the start to the
  * end.
  *
  * This is a complete rewrite of my initial solution.
  *
  * First ... I played with my own implementations of dfs and bfs (see util.Dfs and util.Bfs). This
  * worked. I got the right solution (for part1).
  *
  * Before tackling part2 I decided to switch gears and use scala-graph instead of my own bfs/dfs
  * implementation.
  *
  * Part1:
  *
  ...
  * Part2:
  *
  *   - Read all the files/memorys (into a grid and then into a graph), one by one by one
...
object Day18:
  ...
  def fromResource[G](filename: String, initialGridBytes: Int = Int.MaxValue)(using
    ...
    try
      ...
      val grid = factory
        .create(free, blocked, Some(Position(0, 0)), Some(Position(dimX - 1, dimY - 1)), dimensions)
