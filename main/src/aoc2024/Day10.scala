...
/** Day10 - Hoof It
  ...
  *
  * Just wondering if there is also an angle to build combinations of digits and find the (shortest)
  * ones that start with 0 and end with 9.
  *
  * But (probably) not the way to go ...
  *
  * Let's stick with the walk the map approach ...
  *
  * Note: Let's make minimal preperations for part2 (e.g. detecting loops, finding shortest path,
  * ...) by keep the list of positions that will get us to the end of the trial.
  *
  *   - walk the map to get the positions of all trail-heads
  *   - for every trail head collect all paths/positions
  *   - count the number of unique path for every trail-head (the scores) and sum them up
  *
  * part2:
  *
  * Wow. This time I did hit the jack-pot. I did guess part2 right. I already had everything I need.
  * Just implement the rating method.
  */
  ...
  class TopographicalMap(private val grid: Grid):
    ...
    extension (p: Position)
      ...
      def collect(ps: Set[Position]): Set[Set[Position]] =
        if this(p) == maxHeight then Set(ps + p) else p.next.flatMap(_.collect(ps + p))
      end collect

      /** @return the score for the given position/trailhead */
      def score: Int =
        val allTrials = p.collect(Set())
        val allTrialsWithAUniqueExit = allTrials
          .map(_.filter(p0 => this(p0) == minHeight || this(p0) == maxHeight))
    ...
    def trailHeads: Set[Position] =
      val cells = grid.zipWithIndex.flatMap((line, x) => line.zipWithIndex.map((v, y) => (v, x, y)))
      val heads = cells.withFilter((v, _, _) => v == minHeight).map((_, x, y) => Position(x, y))
      heads.toSet
    end trailHeads
