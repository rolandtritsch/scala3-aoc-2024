...
/** Day17 - Chronospatial Computer
  ...
  *
  * We need to model the concept of Registers (A, B, C) and the Program (a sequence of
  * Instructions).
  *
  ...
  *
  * Means a Program has the current Registers and the current ProgramCounter and the set of
  * Instructions.
  */
...
object Day17:
  ...
  case class ADV(operandLiteral: Int) extends Instruction:
    ...
    override def operand(registers: Registers, literal: Int = operandLiteral): Operand = super
      .operand(registers, literal)
  ...
  case class JNZ(operandLiteral: Int) extends Instruction:
    ...
    override def execute(registers: Registers): Registers =
      if registers('A') == 0 then registers else registers.updated('Z', operandLiteral.toLong)
  ...
  class Program(
    ...
    /** @return
      *   true if the program is finished (has reached the halted state; there is no instruction at
      *   the current counter)
      */
    ...
    def next: Program =
      ...
      val newCounter = if newRegisters.contains('Z') then newRegisters('Z').toInt else counter + 1
      val newOutputs = if newRegisters.contains('Y') then newRegisters('Y') :: outputs else outputs
      new Program(newCounter, newRegisters.removed('Z').removed('Y'), instructions, newOutputs)
    end next

    /** @return
      *   the final Program (State) (after running the Program recursively and it has reached the
      *   halted state)
      */
  ...
  def readFileInstructions(filename: String): Seq[Instruction] =
    ...
    try source.getLines().toSeq.flatMap: line =>
        ...
        val parsed = parser.findAllIn(line).matchData.next.subgroups.head.split(",").map(_.toInt)
          .grouped(2).map(pair => (pair(0), pair(1)))
        logger.debug(s"parsed: ${parsed}")
