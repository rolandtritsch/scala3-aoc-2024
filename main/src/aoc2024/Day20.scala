...
/** Day20 - Race Condition
  ...
  * twist: We are going to create variations of grids to find the "best" one.
  *
  * The (initial) datastructures will will look like the datastructures from
  * Day16.
  *
  * Part1:
  *
  ...
  *     - We (probably) do not need to keep track of the free space (we can just
  *       remove the wall)
  *     - The walls that can be removed are the ones that are on the ones that
  *       have a space in fron of them and a space behind them
...
object Day20:
    ...
    def part1(state: State, threshold: Int = 100): Int =
        ...
        val shortCutsPathLength =
            track.shortCuts(program, shortestPath.get).map: (c, p) => // scalafix:ok
