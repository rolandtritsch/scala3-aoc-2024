...
/** Day12 - Garden Groups
  ...
  *
  * This is going to be interesting. First we have to build/collect the regions. We can do this with
  * flood-fill depth-first traversal.
  *
  * We then need to determine the number of neighbors every cell has. With the number of neighbors
  * we can can calculate the perimeter like this ...
  *
  * For all cells in the region sum up the perimeters of each cell, where the perimeter of each cell
  ...
  *
  * We can then build the regions by traversing the plots and calculate the area and perimeter for
  * each region. A Region would be ...
  *
  ...
  *
  * Means the only thing that we need to figure out is the number of corners per Plot (or per
  * Region).
  *
  * That is unfortunately not as easy as it sounds. Luckily (tongue-in-cheek) soembody(!!!) has done
  * the hard work already, means we can just use the scala-corner library.
  *
  *   - read the plots (including the number of neighbors)
  *   - build the regions (with a recursive dfs flood-fill)
...
object Day12:
  ...
  class Garden(plots: Set[Plot]):
    ...
    val regions: Set[Region] = plotsByPlant.values.foldLeft(Set.empty[Region]): (regions, plots) =>
    ...
    /** @return
      *   the number of sides for the given region (which is given by its Set of Positions).
      *
      * @note
      *   We can use the scala-corner library to count the number of corners, because the number of
      *   corners is the number of sides.
      */
    def countSides(positions: Set[Position], plant: Char): Int =
      ...
      val corners = new corner.CornerCounter(ps.toMap.withDefault(_ => '.')).corners
      corners.map(_._2).sum
    end countSides

    /** @return
      *   (find and return) all regions in the garden, using a flood-fill algorithm.
      */
