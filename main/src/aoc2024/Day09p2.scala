...
/** Day09 - Disk Fragmenter
  ...
  *   - note: I will probably also add the index/address of the block (to allow
  *     for the implementation of a filesystem check)
  ...
  *   - note: after the swap we need to check the block before/after the swapped
  *     blocks and potentially merge the blocks into one (if they are all
  *     freespace blocks)
  ...
  * able to get it to work. The code looked even more ugly than the code that
  * uses a mutable list.
  */
