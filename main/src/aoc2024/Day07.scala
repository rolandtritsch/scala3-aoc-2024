...
/** Day07 - Bridge Repair
  ...
  * part1:
  *
  * Hhhmmm ... I think this can be solved with a tree data-structure and a
  * tree-walking algorithm.
  *
  *   - the root node has the initial value
  *   - you then have one child for '+' and one for '/'
  *   - and so on
  *   - you then try to find a leaf node that equals the number you are looking
  *     for
  ...
  *
  *   - take a given value, take the next one from the list and recurse twice.
  *     Once with '+' and once with '*'
  *   - if one comes back true, we are good
  *
  * part2:
  *
  * Boy ... am I happy that I did not do the premature optimization. This worked
  * like a charm.
  *
