...
object Day23:
  ...
  /** @return the number of all sets of three inter-connected computers where at
    *   least one computer starts with 't'
    */
  def part1(connections: Seq[Connection]): Int =
    ...
    val triplets =
      for
        a <- allNodes.toSeq
        b <- graph(a)
        c <- graph(a).intersect(graph(b)) if a < b && b < c /* Avoid duplicates
         * by enforcing order */
      yield Set(a, b, c)
