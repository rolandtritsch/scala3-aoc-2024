...
/** Day13 - Claw Contraption
  *
  * Let first tackle reading the input file. It comes in blocks of 4 lines ... --- Button A: X+69,
  * Y+23 Button B: X+27, Y+71 Prize: X=18641, Y=10279
  *
  * ---
  *
  * Let's group the lines into groups of 4 and merge the groups into one String and regex on that
  * String to extract the configuration of the ClawMachine.
  *
  ...
  * part1:
  *
  *   - build the first 100 combinations of pressing A and B with a recursive function
  ...
  *
  * Tried running my solution with a depth of Int.MaxValue and ... (you can guess what happened).
  *
...
object Day13:
  ...
  case class ClawMachine(A: Button, B: Button, target: Position):
    ...
    def cheapestWayToWin(depth: Int): Option[Long] =
      ...
      def loop(
        ...
        else if position == target then (ways + way, (minCost, Some(way.sum)).min)
        else if way.sum >= minCost.getOrElse(Long.MaxValue) then (ways, minCost)
        else if visited.contains(way) then (ways, minCost)
        else
          visited += way
          val (waysA, minCostA) = loop(position + A.diff, way + A.cost, ways, minCost, depth - 1)
          val (waysB, minCostB) = loop(position + B.diff, way + B.cost, ways, minCost, depth - 1)
          (waysA ++ waysB, (minCostA, minCostB).min)
        end if
      end loop

      val (_, cheapestWay) = loop(Position(0, 0), MultiSet.empty, Set.empty, None, depth)
    ...
    /** @return
      *   the cheapest way to win the price (by solving the linear equation system)
      */
    def solve: Option[(Long, Long)] =
      def isWhole(n: Double): Boolean = BigDecimal(n).setScale(3, BigDecimal.RoundingMode.HALF_UP)
        .isWhole()

      def isPositive(n: Double): Boolean = n > 0

      val coefficients =
        DenseMatrix((A.diff.x.toDouble, B.diff.x.toDouble), (A.diff.y.toDouble, B.diff.y.toDouble))
      val constants = DenseVector(target.x.toDouble, target.y.toDouble)
      val solution = coefficients \ constants
      assert(solution.length == 2)

      Option.when(solution.forall(n => isWhole(n) && isPositive(n)))(
  ...
  extension (machines: Set[ClawMachine])
    def cheapestWaysToWin(depth: Int): Long = machines.toList.flatMap(_.cheapestWayToWin(depth)).sum
