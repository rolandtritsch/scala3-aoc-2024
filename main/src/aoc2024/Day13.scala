...
/** Day13 - Claw Contraption
  ...
  * ---
  *
  * Let's group the lines into groups of 4 and merge the groups into
  * one String and regex on that String to extract the configuration
  * of the ClawMachine.
  *
  * We need a Position (again). The ClawMachine will be something
  * like this ...
  *
...
object Day13:
  ...
  case class ClawMachine(A: Button, B: Button, target: Position):
    def cheapestWayToWin(depth: Int): Option[Long] =
      ...
      def loop(position: Position, way: MultiSet[Token], ways: Set[MultiSet[Token]], minCost: Option[Long], depth: Int): (Set[MultiSet[Token]], Option[Long]) =
        logger.debug(s"position: ${position}, way: ${way}, ways: ${ways}, minCost: ${minCost}, depth: ${depth}")

        if (depth < 0) (ways, minCost)
        else if (position > target) (ways, minCost)
        else if (position == target) (ways + way, (minCost, Some(way.sum)).min)
        else if (way.sum >= minCost.getOrElse(Long.MaxValue)) (ways, minCost)
        else if (visited.contains(way)) (ways, minCost)
        else
          visited += way
