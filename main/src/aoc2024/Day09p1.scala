...
/** Day09 - Disk Fragmenter
  ...
  *   - lets iterate through the blocks and fill the freespace with blocks from the back (until we
  *     have filled all the freespace)
  *   - note: I was struggling to understand when to stop filling freespace. But then I realized
  *     that I can stop, when I have filled the first N blocks with N being the number of used
  *     blocks
  *   - then we just need to do the checksum
  *   - done
  *
  * Note: Turns out that part2 required some serious refactoring. But I still like this approach and
  * the implementation. Means this time I am creating two objects for part1 and part2.
  */
...
object Day09p1:
  ...
  def readFile(filename: String): Disk =
    ...
    try
      ...
      val (blocks, _) = line.tail.grouped(2).foldLeft(firstBlock, 1): (blocks, chars) =>
