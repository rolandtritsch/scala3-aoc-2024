...
/** Day11 - Plutonian Pebbles
  ...
  *   - to support the splitting of the stones I was (initially) thinking to use a double-linked
  *     list again, but (because) we traverse (and modify) the list (and do not need index access to
  *     the list elements) we probably can (just) use a List
  *   - lets assume that the numbers in the list can get big and use a (at least) a Long (maybe even
  *     a BigInt)
  *   - to walk the list we can use a loop, a map or a recursion. I will start with a map and see
  *     how it goes
  *
  * part1:
  *
  ...
  *   - let's get rid of building/using the (at the end very long) list. Instead we can (just) build
  *     up the number of elements by going down into the recursions. For instance ...
  *     - if you have a stone and in N recursions there are no splits than the number of elements
  *       coming from that one stone is 1
  *     - if there is one split it is 2, with two splits it is 4, ...
  *   - means we do not need to build the list to count the elements
  *   - let's also start to cache results, means if we process a stone on level n somewhere in the
  *     tree, than we do not need to process it again. We can just use the result we already
  *     calculated
  *   - last but not least (and I am not sure, if this is going to have a big impact), we can use
  *     parallel collections to count on all cores in parallel
  */
...
object Day11:
  ...
  extension (stone: Stone)
    ...
    def apply(rules: List[Rule]): (Stone, Option[Stone]) =
      val (s0, s1, _) = rules.foldLeft(0L, Option(0L), false): (applied, rule) =>
      ...
      if n > 0 then
        cache.get(stone, n).getOrElse:
          val i = stone.apply(rules) match
            ...
            case (s0, Some(s1)) => s0.countN(rules, n - 1) + s1.countN(rules, n - 1)
            case _              => throw new RuntimeException("Unexpected case")
          end i
          cache.put((stone, n), i)
          i
  ...
  extension (s: String) def stones: List[Stone] = s.split(" ").toList.map(_.toLong)
