...
trait Dfs:
  ...
  /** @return
    ...
    *
    * @note
    *   The implementation is recursive, but not tail-recursive. Means it might run out of stack
    *   (depending on the size and shape of the grid). And it is slow.
    */
  ...
  /** A global map of visited positions and the shortest path found so far. We need this to cut off
    * the search as soon as we find a path with a higher score than the one we have found so far.
    */
  ...
  /** @return
    ...
    *
    * @note
    *   Cheap is defined by the scoring function. A lower score means a better path. The default
    *   scoring function is the lenth of the path. Means the default is the shortest path.
    *
    * @note
    *   The implementation is recursive, but not tail-recursive. Means it will run out of stack
    *   (fast). And it is slow.
    */
  def findCheapest(start: Position): Option[Path] =
    def findCheapest(current: Position, path: Path, bestPath: Option[Path])(using
        visited: mutable.Map[Position, Int]
