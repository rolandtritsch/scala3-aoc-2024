...
trait Dfs:
  ...
  /** @return the first path found from the start to the end or None
    *
    * @note The implementation is recursive, but not tail-recursive. Means it
  ...
  /** @return the cheapest path found from the start to the end or None
    *
    * @note Cheap is defined by the scoring function. A lower score means a
    *   better path. The default scoring function is the lenth of the path.
    *   Means the default is the shortest path.
    *
    * @note The implementation is recursive, but not tail-recursive. Means it
    *   will run out of stack (fast). And it is slow.
    */
