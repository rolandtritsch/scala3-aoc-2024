...
class GridGraphTest extends munit.ScalaCheckSuite:
    ...
    test("GridGraph - source"):
        ...
        assert(
          !graph.isComplete
        ) // Not every node is connected to every other node
        assert(
          graph.isConnected
        )                      // All traversable positions can reach each other
        ...
        assert(
          !graph.isHyper
        ) // No hyperedges (edges connecting more than 2 nodes)
    ...
    test("GridGraph - shortestPath - small"):
        ...
        val graph = GridGraph.fromGrid(grid)
        val path = graph.get(Position(1, 1))
            .shortestPathTo(graph.get(grid.end.get)).get.nodes
