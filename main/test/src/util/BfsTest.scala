...
class BfsTest extends munit.ScalaCheckSuite:
    ...
    test("Bfs - findFirst"):
        ...
        val pathRecursive = grid
            .findFirstRecursive(Set(List(grid.start.get)), Set(grid.start.get))
        val expected = List(
          Position(1, 1),
          Position(2, 1),
          Position(3, 1),
    ...
    test("Bfs - findFirst - no-boundaries"):
        val grid = Grid
            .fromResource[GridWithBfs]("./tests/GridTest-NoBoundaries.txt")
        val pathIterative = grid.findFirstIterative()
        val pathRecursive = grid
            .findFirstRecursive(Set(List(grid.start.get)), Set(grid.start.get))
    ...
    test("Bfs - findFirst - smallMedium"):
        val grid = Grid
            .fromResource[GridWithBfs]("./tests/GridTest-SmallMedium.txt")
        val pathIterative = grid.findFirstIterative()
        val pathRecursive = grid
            .findFirstRecursive(Set(List(grid.start.get)), Set(grid.start.get))
