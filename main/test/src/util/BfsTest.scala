...
class BfsTest extends munit.ScalaCheckSuite {
  ...
  given Grid.GridFactory[GridWithBfs] with {
    def create(free: Set[Position], blocked: Set[Position], start: Option[Position], end: Option[Position], dimensions: (Int, Int)): GridWithBfs =
      new GridWithBfs(free, blocked, start, end, dimensions)
  }
