...
class Day21Test extends munit.ScalaCheckSuite:
  ...
  test("Day21 - next(029A)"):
    ...
    assertEquals(obtained.map(_.size).min, expected.size)
    assert(obtained.contains(expected))

    val obtained2 = obtained.flatMap(n => next(n, 'A', "", Set()))
    assertEquals(obtained2.size, 98304)
    assertEquals(obtained2.map(_.size).min, results("029A").size)
    assert(obtained2.contains(results("029A")))

  test("Day21 - path(179A)"):
    val pad = NumericKeypad.create
    val obtained = pad.path("179A")
    val expected = "^<<A^^A>>AvvvA"
    assertEquals(obtained, expected)
  ...
  test("Day21 - next(179A)"):
    ...
    val obtained = next(keys, 'A', "", Set())
    val expected = "<Av<AA>>^A<AA>AvAA^A<vAAA>^A"
    assertEquals(obtained.size, 8)
    assertEquals(obtained.map(_.size).min, expected.size)
    assert(obtained.contains(expected))

    val obtained2 = obtained.flatMap(n => next(n, 'A', "", Set()))
    assertEquals(obtained2.size, 49152)
    assertEquals(obtained2.map(_.size).min, results("179A").size)
    assert(obtained2.contains(results("179A")))

  test("Day21 - level(029A)"):
    val pad = NumericKeypad.create
    val keys = pad.path("029A")
    val obtained = level(keys, 1)
    assertEquals(obtained, 68)

  test("Day21 - level(980A)"):
    val pad = NumericKeypad.create
    val keys = pad.path("980A")
    val obtained = level(keys, 1)
    assertEquals(obtained, 60)

  test("Day21 - level(179A)"):
    val pad = NumericKeypad.create
    val keys = pad.path("179A")
    val obtained = level(keys, 1)
    assertEquals(obtained, 68)

  test("Day21 - level(456A)"):
    val pad = NumericKeypad.create
    val keys = pad.path("456A")
    val obtained = level(keys, 1)
    assertEquals(obtained, 64)

  test("Day21 - level(379A)"):
    val pad = NumericKeypad.create
    val keys = pad.path("379A")
    val obtained = level(keys, 1)
    assertEquals(obtained, 64)
