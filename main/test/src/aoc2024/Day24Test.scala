...
class Day24Test extends munit.ScalaCheckSuite {
  ...
  test("Day24 - assignment - toString") {
    assertEquals(Assignment(LeftVariable("a"), Value(true)).toString, "def a = true")
    assertEquals(Assignment(LeftVariable("b"), Expression(LeftVariable("c"), Operation.AND, RightVariable("d"))).toString, "def b = c && d")
  }

  test("Day24 - generate - test2") {
    val initials = readFileInitials("./inputs/Day24Test2-Initials.txt")
    val statements = readFileStatements("./inputs/Day24Test2-Statements.txt")
