...
class Day20Test extends munit.ScalaCheckSuite:
  ...
  test("Day20 - shortCuts - shortCutsPathLength - test"):
    ...
    val shortCutsPathLength = track.shortCuts(program, shortestPath.get).map:
      case (c, p) => (c, p.getOrElse(List.empty).length)
  ...
  test("Day20 - shortCuts - final - test"):
    ...
    val obtained = shortCutsValue.groupBy(_._2)
      .map((cheatValue, cheatList) => (cheatValue, cheatList.size)).toSet
